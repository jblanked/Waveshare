...
int main()
{
    ...
    lcd_fill_rect(60, 140, 100, 60, COLOR_RED);                    // Red filled rectangle
    lcd_draw_rect(200, 20, 80, 60, COLOR_GREEN);                   // Green rectangle outline
    lcd_fill_circle(360, 160, 20, COLOR_BLUE);                     // Blue filled circle
    lcd_draw_circle(240, 160, 20, COLOR_YELLOW);                   // Yellow circle outline
    lcd_draw_line(20, 240, 460, 240, COLOR_CYAN);                  // Cyan horizontal line
    lcd_fill_triangle(240, 300, 200, 380, 280, 380, COLOR_VIOLET); // Violet triangle
    lcd_draw_text(100, 260, "RP2350 Touch LCD Demo", COLOR_WHITE); // White text
    ...
    while (rotations--)
    {
        ...
        while (frame < 3)
        {
            ...
            lcd_blit(220, 220, 16, 15, frame == 0 ? frame_0 : (frame == 1 ? frame_1 : frame_2));
            lcd_swap();
            sleep_ms(200);
            frame++;
        }
